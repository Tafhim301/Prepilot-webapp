...
function PlanCard({ plan, annual, index }: { plan: PricingPlan; annual: boolean; index: number }) {
  ...
  return (
    <motion.div
      ...
      style={
        H
          ? { ...cardDark, background: P.darkBg, border: "1px solid rgba(255,255,255,0.08)", boxShadow: `0 32px 64px -16px rgba(0,0,0,0.45), 0 8px 24px -8px ${P.red}35` }
          : { ...cardGlass, boxShadow: `0 4px 24px rgba(0,0,0,0.07), 0 1px 4px rgba(0,0,0,0.04)` }
      }
      onMouseEnter={(e) => {
        if (!H) (e.currentTarget as HTMLElement).style.boxShadow = cardHoverShadow;
      }}
      onMouseLeave={(e) => {
        if (!H) (e.currentTarget as HTMLElement).style.boxShadow = `0 4px 24px rgba(0,0,0,0.07), 0 1px 4px rgba(0,0,0,0.04)`;
      }}
    >
      {H && (
        <div
          ...
          style={{ background: `radial-gradient(ellipse 80% 50% at 10% 0%, ${P.red}30 0%, transparent 55%)` }}
        />
      )}
      ...
      <div className="px-8 pt-8 pb-6 relative z-10">
        ...
        <div className="flex items-baseline gap-1.5">
          <span className="text-xl font-medium" style={{ color: H ? P.onDarkMid : P.inkMid }}>$</span>
          ...
          <span className="text-sm font-medium pb-1" style={{ color: H ? P.onDarkMid : P.inkMid }}>/mo</span>
        </div>
        ...
        <p className="text-sm mt-5 leading-relaxed" style={{ color: H ? P.onDarkMid : P.inkMid }}>
          {plan.tagline}
        </p>
      </div>
      ...
      >
        {[
          ...
          <div key={d.label} className="flex items-center gap-1.5" style={{ color: H ? P.onDarkMid : P.ink }}>
            <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: H ? P.amber : P.primary }} />
            {d.label}
          </div>
        ))}
      </div>
        ...
        {plan.features.map((f) => (
          <div key={f.label} className="flex items-start gap-3">
            <div
              ...
              style={
                f.included
                  ? { background: H ? GRAD : `${P.primary}18` }
                  : { background: "transparent", border: `1.5px dashed ${H ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.12)"}` }
              }
            >
              {f.included
                ? <Check className="w-2.5 h-2.5" strokeWidth={3} style={{ color: H ? P.onDark : P.primary }} />
                : <X     className="w-2.5 h-2.5" strokeWidth={2.5} style={{ color: H ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.2)" }} />
              }
            </div>
            <span
      ...
      <div className="px-8 pb-8 relative z-10">
        <Link
          ...
          style={
            H
              ? { background: GRAD, color: P.onDark, boxShadow: `0 8px 24px -6px ${P.red}55` }
...
function Addons() {
  ...
  return (
    <div ref={ref} className="mt-24 max-w-4xl mx-auto">
      ...
      >
        <p className="text-[11px] font-bold uppercase tracking-[0.18em] mb-2" style={{ color: P.primary }}>
          Extend your scope
        </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {data.addons.map((addon, i) => (
          <motion.div
            ...
            style={cardGlass}
          >
            <span className="text-sm font-semibold" style={{ color: P.ink }}>{addon.name}</span>
            <span
...
function FaqItem({ faq, index }: { faq: { question: string; answer: string }; index: number }) {
  ...
  return (
    <motion.div
      ...
      style={{
        background:    open ? "rgba(255,255,255,0.80)" : "rgba(255,255,255,0.50)",
        border:        `1px solid ${open ? "rgba(255,255,255,0.95)" : "rgba(255,255,255,0.65)"}`,
    ...
    >
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <p className="pb-5 px-6 text-sm leading-relaxed" style={{ color: P.inkMid }}>
              {faq.answer}
            </p>
          </motion.div>
        )}
  ...
  return (
    ...
    >
      ...
      <div aria-hidden className="absolute inset-0 pointer-events-none overflow-hidden">
        <div
          className="absolute -top-40 left-1/2 -translate-x-1/2 w-[800px] h-[500px] rounded-full blur-[130px] opacity-35"
          style={{ background: `radial-gradient(ellipse, ${P.red}, transparent 70%)` }}
        />
        <div
          className="absolute bottom-0 right-0 w-[500px] h-[500px] rounded-full blur-[110px] opacity-28"
          style={{ background: `radial-gradient(circle, ${P.amber}, transparent 65%)` }}
        />
        <div
          className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full blur-[100px] opacity-18"
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          ...
          <p className="mt-5 text-lg max-w-lg mx-auto leading-relaxed" style={{ color: P.inkMid }}>
        ...
        <motion.div
          ...
          style={{ background: P.darkBg, boxShadow: `0 24px 60px -12px rgba(0,0,0,0.4), 0 8px 24px -8px ${P.red}22` }}
        >
          <div
            aria-hidden
            className="absolute inset-0 pointer-events-none"
            style={{ background: `radial-gradient(ellipse 60% 80% at 90% 10%, ${P.red}28 0%, transparent 50%)` }}
          />

          <div className="relative flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="text-center md:text-left">
              <h4 className="text-2xl sm:text-3xl font-bold tracking-tight" style={{ color: P.onDark }}>
                Not sure which plan fits?
              </h4>
              <p className="mt-3 text-base leading-relaxed max-w-sm" style={{ color: P.onDarkMid }}>
            ...
            <Link
              ...
              style={{ background: GRAD, color: P.onDark, boxShadow: `0 8px 24px -6px ${P.red}60` }}
            >
              Book a Free Call
              <ArrowRight className="w-4 h-4 transition-transform duration-200 group-hover:translate-x-0.5" />
            </Link>
