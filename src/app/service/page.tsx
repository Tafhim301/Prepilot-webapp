...
function ServiceCard({ icon, title, description, tags }: { icon: string; title: string; description: string; tags: string[] }) {
  return (
    ...
    >
      ...
      <div aria-hidden className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
      ...
      <div className="w-14 h-14 rounded-xl flex items-center justify-center text-2xl select-none z-10 transition-transform duration-300 group-hover:scale-110"
        style={{ background: "rgba(160,48,200,0.15)" }}>{icon}</div>
      ...
      <div className="flex flex-wrap gap-2 z-10">
        {tags.map((tag) => (
          <span key={tag} className="text-xs rounded-full px-3 py-1"
            style={{ border: `1px solid ${P.border}`, color: P.inkLight, background: P.mutedFill }}>{tag}</span>
        ))}
      </div>
      <span className="inline-flex items-center gap-1 text-sm font-semibold mt-auto z-10 w-fit transition-opacity hover:opacity-70" style={GRAD_TEXT}>
        See More <ArrowUpRight size={15} className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
      </span>
    </motion.div>
  );
}
  ...
  return (
    <div className="w-full px-4 sm:px-8 md:px-12 lg:px-16 py-16 sm:py-20 flex flex-col gap-16 relative overflow-hidden"
      ...
      <div aria-hidden className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-24 -right-24 w-[480px] h-[480px] rounded-full blur-[110px] opacity-16"
          style={{ background: `radial-gradient(circle, ${P.violet}, transparent 65%)` }} />
        <div className="absolute -bottom-20 -left-20 w-[380px] h-[380px] rounded-full blur-[100px] opacity-12"
      ...
      <div className="flex flex-col lg:flex-row gap-10 items-start relative z-10">
        ...
        <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, ease }}
          className="lg:sticky lg:top-24 w-full lg:w-[300px] xl:w-[340px] flex-shrink-0 flex flex-col gap-6 rounded-2xl p-7 relative overflow-hidden"
          style={cardGlass}>
          <div aria-hidden className="absolute top-[-30%] right-[-20%] w-[200px] h-[200px] rounded-full pointer-events-none"
          ...
          <motion.ul className="relative z-10 flex flex-col gap-2" variants={listVariants} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            {summaryText.map((text) => (
              <motion.li key={text} variants={listItemVariants}
                className="flex items-center gap-2.5 group/item p-1 rounded-lg hover:bg-white/5 transition-colors cursor-default">
                <Check className="w-3.5 h-3.5 flex-shrink-0" style={{ color: P.purple }} strokeWidth={2.5} />
                <span className="text-sm font-medium transition-colors group-hover/item:text-white/90" style={{ color: P.inkMid }}>{text}</span>
              </motion.li>
            ))}
          </motion.ul>
        ...
        {/* Cards grid */}
        <motion.div className="grid grid-cols-1 sm:grid-cols-2 gap-5 flex-1" variants={containerVariants} initial="hidden" animate="visible">
